...
**Next Prompt A feature:** None. RG6 is held/product-red after fresh evidence; the immediate Matt-directed production plumbing remediation for readback/source-assisted-proof/readiness is implemented on `fix/prod-readback-source-proof-readiness`, but no public SaaS/account/billing work or yellow/green claim is unlocked without a later reviewed RG6 decision clearing the remaining live blockers.
