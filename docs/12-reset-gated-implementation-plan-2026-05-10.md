...
**Current reset gate:** RG6 - Dogfood / Kill Decision remains held/product-red. Prompt C accepted the R15 red-hold recommendation; Matt authorized fresh production evidence on 2026-05-22; the Matt-directed lead-quality/contact-yield remediation merged to `main`; the 2026-05-24 post-remediation Prompt C re-audit kept RG6 held/product-red; the later unblocked remediation recheck improved runtime/contact/timed-export evidence but still did not clear RG6; RG6R3 improved Arizona source/persona/org precision without clearing the gate; and the first RG6R4 Prompt B run failed on external Tavily quota before merge. Tavily quota is restored/verified as of 2026-05-26, so RG6R4 is unblocked for a fresh Prompt B rerun.
**Next Prompt A feature:** None while RG6R4 is waiting for Prompt B rerun. No public SaaS/account/billing work or yellow/green claim is unlocked.
**Current Prompt B handoff:** Re-run `RG6R4 - required-suite contact precision and web-boundary follow-up` on `fix/rg6r4-required-suite-contact-precision` now that Tavily quota is restored. The 2026-05-25 Prompt B run passed local core/API/web checks and preserved web/API auth boundaries, but did not merge because the required-suite product cases returned `503 tavily_failed`, the sampled packet had `0` rows, and contact precision did not improve. The branch now also includes follow-up internal-token API middleware, contact-query precision, and sampled-precision denominator fixes; the next Prompt B must rerun against the updated branch, capture fresh required-suite evidence, and prove contact precision improves before merge.
...
| RG6R4 | Required-suite contact precision and web-boundary follow-up | implemented_pending_qa | `fix/rg6r4-required-suite-contact-precision` | core/API/web passed; Tavily quota restored, Prompt B rerun required |
...
- Historical queue consequence: RG6R3 restored normal two-prompt A/B rhythm and assigned Prompt A to `RG6R4 - required-suite contact precision and web-boundary follow-up`. Prompt A has now implemented RG6R4, so the current pointer is Prompt B QA on `fix/rg6r4-required-suite-contact-precision`. No Prompt C is required until Matt decides the narrow remediation tranche is complete and asks for a gate-level RG6 decision.
...
RG6R4 2026-05-26 follow-up after blocked Prompt B:

- Branch: `fix/rg6r4-required-suite-contact-precision`.
- Status: `implemented_pending_qa`; Tavily quota is restored, so Prompt B should rerun on the updated branch now. Do not merge until Prompt B passes with fresh required-suite evidence.
- Why it exists: The 2026-05-25 Prompt B artifacts preserved useful local/web-boundary evidence, but the merge condition failed before product evidence could prove contact precision because Tavily quota blocked required-suite searches. This follow-up keeps that failed-QA evidence and tightens the branch so the next Prompt B rerun has the right web-boundary and sampled-precision semantics.
- What changed after the failed Prompt B run: Contact-evidence queries now prefer known official domains, spend at most one query on third-party evidence domains, and add broad employer/name contact searches when the seed source is not an employer domain. Direct and pattern contact promotion now accepts employer-domain evidence only when the source domain and page text back the candidate organization; unrelated/generic domains remain blocked. Sampled precision packets now count only person lead rows in precision denominators while retaining organization-only/not-found/failed rows as context. Web middleware now accepts `x-white-rabbit-internal-token` on protected API paths when it matches `WR_API_INTERNAL_TOKEN`, without bypassing shared-session auth for pages.
- Retrospective sanity check: Running sampled precision against the RG6R3 direct artifacts with the updated denominator produced `sampled_row_count=48`, `precision_scored_row_count=18`, `context_row_count=30`, persona `0.667`, organization `0.833`, source `0.944`, contact `0.389`, unsupported email `0`, fake email `0`, and `green_precision_floor_met=false`. This is not a gate-clearing artifact.
- Follow-up verification passed: `cd packages/core && PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run pytest tests/test_contact_evidence.py tests/test_sampled_precision.py -q --tb=short` (`11 passed`); `cd packages/core && PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run pytest tests -q` (`177 passed, 6 skipped`); `cd apps/api && WR_API_INTERNAL_TOKEN=test-internal-token uv run pytest tests -q` (`57 passed`, existing datetime warnings); `cd apps/web && npm test -- --run src/middleware.test.ts src/app/api/scout/route.test.ts src/app/api/source-assisted-proof/route.test.ts` (`3 files`, `8 tests`); `cd apps/web && npm test -- --run` (`16 files`, `40 tests`); `cd apps/web && npm run build` passed with the existing deprecated `middleware` warning; `git diff --check` passed.
- Tavily quota verification: After Matt added credits on 2026-05-26, the live Fly `TAVILY_API_KEY` usage endpoint reported Project plan with `plan_limit=4000`, `plan_usage=3499`, and `paygo_limit=null`; a live advanced Tavily search smoke returned HTTP 200. Treat the prior `qa_failed` run as historical non-merge evidence, not the current blocker.
- Exact Prompt B rerun handoff: Re-run now. Verify the diff is limited to contact precision, sampled precision packet semantics, and internal-token API middleware. Rerun `git diff --check`, `cd packages/core && PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run pytest tests -q`, `cd apps/api && WR_API_INTERNAL_TOKEN=test-internal-token uv run pytest tests -q`, `cd apps/web && npm test -- --run`, and `cd apps/web && npm run build`. Capture fresh web-boundary evidence proving `/api/scout` and `/api/source-assisted-proof` accept the internal token and reject missing tokens. Capture fresh required-suite outputs and sampled precision after this branch is deployed or run against the same service boundary Prompt B will merge from. If QA passes, write/update the QA report, merge only to `main`, and keep RG6 product-red unless a later Prompt C decision changes it.
- Queue consequence: Prompt B remains current and unblocked for rerun. No new Prompt A feature, Prompt C gate decision, yellow/green claim, Thomas/Lee dogfood expansion, public SaaS, account, org, or billing work is valid until RG6R4 is QAed and merged or explicitly failed by Matt.
