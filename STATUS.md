...
**Next pointer:** Deploy/recheck `main` if needed to confirm the guardrail remediation in production. This clears only the guardrail false-positive where a detailed Arizona named-account K-12 technology decision-maker query was marked `needs_more_detail`. Product remains red after this remediation: public SaaS/account/org/billing work plus yellow/green and Thomas/Lee dogfood claims remain blocked. Any next quality work must still be a Matt-directed red remediation scoped to remaining live RG6 product blockers in `audits/raw/reset-2026-05-10/rg6/fresh-2026-05-22/fresh-evidence-summary.md` and then re-audited against `docs/00-product-northstar.md` line by line.

**Assignment lock:** `docs/reset-current-assignment.json` is the machine-readable current assignment. It now records the Matt-directed RG6 guardrail false-positive remediation as merged to `main` while RG6 remains held/product-red; it must agree with any future Prompt A/B/C request before an agent edits files.
...
**Current gate:** Red with Matt-directed Thomas/Lee internal-use exception only. RG5 advanced for export/persistence mechanics, but fresh RG6 evidence confirms live data-quality and dogfood readiness are still not proven; do not treat the promotion as a public launch or as evidence that the full reset passed.
...
**Next feature pointer:** None after RG6 fresh evidence hold. RG6/R15 plus the 2026-05-22 fresh production addendum record a red-hold/no-dogfood decision; no yellow/green, Thomas/Lee dogfood expansion, or public SaaS/account/org/billing work is valid without a later reviewed gate that clears the live blockers line by line against `docs/00-product-northstar.md`.
...
**Latest gate acceptance:** W4 accepted on 2026-05-10. W5 remains explicitly held on `rebuild/validated-leads-loop`; RG0 advanced on 2026-05-10 as a control-plane reset audit; RG1 advanced on 2026-05-10 as a benchmark-harness audit; RG2 advanced on 2026-05-11 as a search/source coverage audit; RG3 advanced on 2026-05-12 after the post-R09L source-assisted live proof audit; RG4 advanced on 2026-05-12 after the sales-first operator UI audit; RG5 advanced on 2026-05-22 after export/persistence audit; RG6 is held/product-red after Prompt C; Matt accepted that held state on 2026-05-22, then authorized a fresh evidence run that confirmed the red hold remains. W6 and all downstream public/dogfood expansion remain blocked until the visible operator loop is proven:
...
**Mainline override:** ADR-024 supersedes the prior "feature branches merge to rebuild first" rule. `main` is now both the operator-use deployment line and the reset integration branch, but the repo must still preserve the red-gate caveats, evidence requirements, internal-only scope, and Prompt A/B/C gate discipline.
...
Next pointer: Deploy/recheck `main` if needed, then confirm the Arizona remediation query reports `query_guardrail.status=clear` in production. Product remains red: this does not clear Arizona lead quality/contact yield, sampled precision, or operator-minute blockers.
...
Status: `merged_deployed_recheck_passed`
...
Verification: API targeted regression tests passed (`test_run_leads_endpoint_does_not_touch_detached_run_after_session_closes`, `test_readiness_tavily_config_only_probe_does_not_degrade_service`, existing run-leads readback and source-assisted API tests). `apps/web` production build passed and lists `/api/source-assisted-proof`. GitHub Actions production deploy run `26306479033` passed: tests, Fly API deploy, and Vercel web deploy all succeeded. Local Vitest route tests could not execute in this shell because the Vitest worker pool timed out before loading any tests, including pre-existing `src/app/api/scout/route.test.ts`; GitHub Actions `npm test` passed on Linux/Node 22. Matt directed that the requested production plumbing recheck be treated as passed: `/readiness` reports ready, Tavily readiness is ready/config_only, web `/api/source-assisted-proof` no longer 404s, and fresh persisted-run readback no longer returns 500.
Next pointer: Product remains red until lead quality/contact yield/operator-minute blockers clear in a reviewed gate. The production plumbing recheck is closed as passed per Matt direction; do not reopen readiness/source-assisted-proof/readback as current blockers unless fresh evidence regresses them.
...
Status: `gate_hold` / product red after fresh production evidence
Report: `audits/gates/reset-2026-05-10/rg6-dogfood-decision.md`
Raw evidence: `audits/raw/reset-2026-05-10/rg6/r15-evidence-summary.md`; `audits/raw/reset-2026-05-10/rg6/prompt-c-verification.txt`; `audits/raw/reset-2026-05-10/rg6/fresh-2026-05-22/`
...
Why it exists: RG6 needed Prompt C and then a Matt-authorized fresh evidence addendum to decide whether the product earns Matt-only yellow evaluation, Thomas/Lee dogfood, continued red hold, or kill/manual-concierge fallback.
What changed: Fresh production evidence was gathered against Vercel/Fly on 2026-05-22. It confirmed working health, shared-password auth, tokenless protected-route rejection, privacy-sensitive query blocking, and visible non-ready tiering, but also found degraded readiness, no web source-assisted proof route, Arizona K-12 Scout output with 0 high-trust usable rows / 0 source-backed contacts, and a separate persisted-run readback 500.
Decision: Hold and keep product red. Do not mark yellow, green, public launch, or Thomas/Lee dogfood readiness.
Evidence summary: The fresh Arizona K-12 run returned 13 categorized rows in 126.569 seconds with estimated cost $0.355318: 6 review person rows, 2 organization-only, 1 not_found, 4 failed, 0 high_trust_usable, 0 contact quality passes, and 0 contacts acquired. Inline persistence readback claimed 13/13 persisted rows, but `GET /api/runs/{run_id}/leads` through the web proxy returned 500.
Verification: Fresh artifact collection wrote sanitized JSON/Markdown under `audits/raw/reset-2026-05-10/rg6/fresh-2026-05-22/`; `git diff --check` must pass before this addendum lands.
Next pointer: No next Prompt A assignment is valid from this gate hold. Keep public SaaS/account/org/billing work and yellow/green/Thomas-Lee dogfood claims blocked. Any next work must be a Matt-directed red remediation scoped to the fresh live blockers and followed by a new reviewed gate record.
Open questions: RG6 still needs remediation and re-audit for production readiness degradation, web source-assisted proof exposure, persisted-run readback 500, Arizona benchmark quality/contact yield, guardrail false-positive detail detection, sampled benchmark precision, and a timed unassisted operator run before any dogfood upgrade.
...
Prompt B result: QA passed. Report: `.gstack/qa-reports/qa-report-r14b-ui-ux-consistency-2026-05-12.md`. Prompt B verified `git diff --check`, `git diff --check main...HEAD`, web Vitest (`13` files, `30` tests), production build, API suite (`53 passed`, existing datetime warnings), saved browser screenshots, mobile `scrollWidth=390` at `390` width, northstar alignment, and R14B-only scope. Prompt B also made a narrow UI-copy fix so shared evidence-drawer correction copy says saved run/search instead of leaking Full-mode wording into the primary operator path.
Next pointer after merge: Prompt A for `R14C - Deployment readiness and operator-use smoke` on `feat/reset-r14c-deployment-readiness-smoke`. RG5 Prompt C, RG6, dogfood, backend/core/export/persistence logic changes, public SaaS work, and R15 remain blocked.
...
What changed: Added `apps/web/src/components/brand-identity.tsx`, updated primary shell and login to use wordmark-first identity, replaced the app favicon and added approved public icon/manifest/social assets, deleted old generated rabbit/lens/rabbit-mark rasters from `apps/web/public/brand/`, updated metadata and middleware so approved browser/social assets are routable, and updated the historical brand draft docs to prevent deleted generated assets from being revived.
Required verification from Prompt A:
- `cd apps/web && npm test -- --run` (`13` files, `30` tests passed)
...
- `cd apps/api && WR_API_INTERNAL_TOKEN=test-internal-token uv run pytest tests -q` (`53 passed`, existing datetime deprecation warnings)
- Browser QA with Playwright on local dev: captured desktop/mobile login, empty, and results screenshots under `.gstack/qa-reports/screenshots/r14a-brand-cleanup-2026-05-12/`; desktop `scrollWidth=1440` at `1440` width; mobile `scrollWidth=390` at `390` width
- `git diff --check` (passed)
Exact Prompt B handoff: QA `feat/reset-r14a-image-overhaul-brand-cleanup`; confirm only R14A brand/image cleanup landed; rerun the required tests/build/API/diff checks; inspect the R14A screenshots; confirm no standalone rabbit/mascot/ad hoc CSS logo appears in the product UI; confirm the old generated rasters are deleted and unreferenced; confirm approved favicon/touch/manifest/social assets are present and public; confirm login, empty, and results states remain usable on desktop/mobile. If QA passes, merge only to `main`, mark R14A `merged_to_mainline`, mark R14B `ready`, and keep R14C/RG6/dogfood blocked.
Open questions: None for R14A. Standalone rabbit/icon remains quarantined pending approved production vector and ADR.
...
Scope: Implement only API/DB persistence and readback support for Scout/Full. Preserve sales-first CSV/export behavior and keep R14A-R14C, RG6, dogfood, and deployment promotion blocked.
Non-goals: No image overhaul, UI/UX consistency pass, deployment smoke, browser visual changes, source-assisted compiler work, benchmark changes, dogfood packet, or RG5 Prompt C.
...
What changed: Reused the existing web export builder for primary-mode Scout results, added a primary-path `Sales-first workbook` export panel below the results overview, reordered CSV headers so the first 10 columns are sales-useful (`lead_name`, `title`, `organization`, `email`, `email_status`, `phone`, `phone_status`, `usable_candidate`, `operator_label`, `candidate_category`), and changed export row building so READY/usable rows sort first while REVIEW/ORG-ONLY/NOT FOUND rows retain validation notes, source URLs, statuses, and run context.
Required verification:
- `cd apps/web && npm test -- --run` (`13` files, `30` tests passed)
- `cd apps/web && npm run build` (passed; existing Next.js warnings about workspace-root inference and deprecated `middleware` naming)
- `cd apps/api && WR_API_INTERNAL_TOKEN=test-internal-token uv run pytest tests -q` (`51 passed`, existing datetime deprecation warnings)
- Browser QA with Playwright on `http://localhost:3000/` using local test auth and mocked `/api/scout` responses: exported a 51-row CSV, confirmed the first 10 headers are sales-first, confirmed READY rows sort first, confirmed REVIEW rows preserve missing contact status, and confirmed no 390px mobile horizontal overflow (`scrollWidth=390`, `innerWidth=390`)
- `git diff --check` (passed)
...
Historical Prompt B handoff: completed; do not rerun R13 QA unless Matt explicitly asks for a regression audit.
Prompt B result: QA passed. Report: `.gstack/qa-reports/qa-report-r13-sales-first-export-2026-05-12.md`. Verified `git diff --check`, web Vitest (`13` files, `30` tests), production build, API suite (`51 passed`), browser/CSV artifact evidence, northstar alignment, and R13-only scope. R13 is live on the promoted mainline.
Next pointer after merge: R14 Prompt A was completed on `feat/reset-r14-persistence-quality-tieout`; run Prompt B next. R14A-R14C, RG6, and dogfood remain blocked.
Open questions: None.
...
- Product is in audit-red state. R13-R15 are merged to `main`; RG0-RG5 have advanced; RG6 is held/product-red after Prompt C and the Matt-authorized fresh production evidence addendum. No downstream Prompt A/B/C assignment is valid unless Matt explicitly authorizes a scoped red remediation for the fresh live blockers. W6 remains blocked.

## Next concrete task

- None from this gate hold. Keep public SaaS/account/org/billing work, yellow/green promotion, and Thomas/Lee dogfood expansion blocked. The next valid action requires Matt to explicitly authorize a scoped red remediation for the fresh live blockers, then re-audit RG6 against the northstar.
