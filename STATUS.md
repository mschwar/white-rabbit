...
**Next pointer:** Prompt A should implement R09 - Tier summary, score semantics, and reason language reset on `feat/reset-r09-tier-summary-semantics` after the R08 merge lands on `rebuild/validated-leads-loop`. Do not unlock RG4 or sync `main`.
...
**Next feature pointer:** R09 - Tier summary, score semantics, and reason language reset is the next same-gate Prompt A feature after the R08 merge lands on `rebuild/validated-leads-loop`. Do not unlock RG4 or sync `main`.
...
**Current feature branch QA status:** R08 passed Prompt B QA on `feat/reset-r08-tier-validation-conflicts` and is being merged to `rebuild/validated-leads-loop`. R07 is merged; R09 is ready as the next same-gate feature after the R08 merge lands.
...
Exact Prompt A handoff: Implement R09 on `feat/reset-r09-tier-summary-semantics` after R08 is merged to `rebuild/validated-leads-loop`. Keep scope to tier summary, score semantics, and reason language reset; do not start RG4, do not unlock the next gate, and do not touch `main`.
Next pointer: Prompt A for R09 after the R08 merge lands.
...
- Product is in audit-red state. Documentation authority remediation is complete; F01-F19 are merged to `rebuild/validated-leads-loop`, but the May 10 audit found the visible loop still fails live operator benchmarks. W2, W3, and W4 are orchestrator-accepted. R00-R08 are merged or being merged through Prompt B; RG2 advanced as a search/source coverage gate; RG3 is in progress with R09 ready next; W5 remains held; W6 remains blocked.

## Next concrete task

- Run Prompt A for R09 - Tier summary, score semantics, and reason language reset on `feat/reset-r09-tier-summary-semantics` after the R08 merge lands on `rebuild/validated-leads-loop`. Do not unlock RG4 or sync `main`.
