...
**Last updated:** 2026-05-11 by Codex prompt-a-r06-nonperson-coverage
...
**Next pointer:** Prompt B should QA `R06 - Not-found and organization-only coverage writer` on `feat/reset-r06-nonperson-coverage`. Do not start RG3.
...
**Next feature pointer:** `R06 - Not-found and organization-only coverage writer` is implemented on `feat/reset-r06-nonperson-coverage` and is waiting for Prompt B QA. Do not start RG3.
...
**Current feature branch QA status:** `feat/reset-r06-nonperson-coverage` is the single reset feature branch waiting for Prompt B QA. R05 is already merged to `rebuild/validated-leads-loop`. No other reset feature branch should be QA'd.
...
What changed: Prompt A implemented the single ready RG2 feature. A new core coverage writer reads named-account obligations from the query plan after extraction and appends only explicit non-person rows for any uncovered account. If the R05 source snapshot contains account evidence, the writer appends an `organization_only` row with the supporting source URL; if no collected source supports the account, it appends a `not_found` row. Existing person, organization-only, not-found, or failed rows that already cover the account are not duplicated. The strict person-lead evidence gate, extraction/scoring logic, UI, export, and RG3 tiering semantics were not changed.
Tests or QA run:
- `cd packages/core && uv run pytest tests/test_query_planner.py tests/test_search.py tests/test_benchmark_suite.py tests/test_coverage.py tests/test_orchestrator.py -q` (`34 passed`)
Screenshots or report: non-UI feature; no browser screenshots required.
...
Exact Prompt B handoff: QA `R06 - Not-found and organization-only coverage writer` on `feat/reset-r06-nonperson-coverage`. Required verification: `cd packages/core && uv run pytest tests/test_query_planner.py tests/test_search.py tests/test_benchmark_suite.py -q`; also run `git diff --check`, confirm the coverage writer only appends explicit `organization_only` / `not_found` rows for uncovered named-account obligations, and do not unlock RG3. If QA passes, merge only to `rebuild/validated-leads-loop` and mark RG2 ready for Prompt C audit because R06 is the last RG2 feature.
Next pointer: Prompt B for R06. Do not start RG3.
