...
export async function GET(request: NextRequest, { params }: { params: Promise<{ recipe_id: string }> }) {
  ...
  const apiUrl = `${getApiBaseUrl()}/recipes/${recipe_id}/scoreboard`;
  ...
  if (!upstreamResponse.ok) {
    return NextResponse.json({ error: 'Failed to fetch recipe scoreboard.' }, { status: upstreamResponse.status });
  }
